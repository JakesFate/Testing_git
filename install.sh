...
copyright(){
    ...
    output "By JakesFate --- Discord: JakesFate#0514"
    output ""
...
get_distribution(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if [ "$dist_version" != "19.04" ] && [ "$dist_version" != "19.10" ] && [ "$dist_version" != "18.10" ] && [ "$dist_version" != "18.04" ] && [ "$dist_version" != "16.04" ] && [ "$dist_version" != "14.04" ]; then
            output "Unsupported Ubuntu version. Only Ubuntu 19.10, 19.04, 18.10, 18.04, 16.04, and 14.04 are supported."
            exit 0
        fi
	elif [ "$lsb_dist" = "debian" ]; then
        if [ "$dist_version" != "10" ] && [ "$dist_version" != "9" ] && [ "$dist_version" != "8" ]; then
            output "Unsupported Debian version. Only Debian 10, 9 and 8 are supported.."
            exit 0
		fi
        ...
        if [ "$dist_version" != "7" ] && [ "$dist_version" != "8" ]; then
            output "Unsupported CentOS version. Only CentOS 7 and 8 are supported."
            exit 0
        fi
